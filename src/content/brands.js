...
// Normalize all brands in the array for consistent matching.
const normalizedBrandsArray = brandsArray.map(brand => normalizeBrand(brand));

// We use a Set for O(1) lookups, which is crucial for performance with large lists.
const SUPPORTED_BRANDS_SET = new Set(normalizedBrandsArray);

// We also expose the original array for operations that require iteration, like `startsWith`.
// This should also be the normalized version for consistency.
const SUPPORTED_BRANDS_ARRAY = normalizedBrandsArray;
